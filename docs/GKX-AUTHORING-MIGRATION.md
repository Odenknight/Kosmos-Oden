# GKX Human-Editable Formatting Behavior

Kosmos-Oden beta.12 uses compact flat GKX 2.2 Obsidian Properties as the
...
GKX 2.3 remains the stricter read-only validating projection unless the user
...
2.2** also onboards mechanically recoverable ordinary, reserved, and
unversioned notes. **Convert all to editable 2.3** onboards mechanically
recoverable notes into the flat editable GKX 2.3 profile: the same
...
scalars) under `gkx_version: "2.3"`. No nested governance blocks are written
into notes; the validating projection supplies spec-permitted in-memory
defaults, and governed values belong in `.gkx/` sidecars. Hand-authored nested
