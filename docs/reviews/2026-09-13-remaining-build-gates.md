...
| --- | --- | --- |
| R0 / R3 exact runtime and Hermes | Installed `78abe88` with Engine `b309d5e`; current Hermes discovery, search, allowed UID, policy-consistent read, observed traversal and warm read passed with stable process/hash. | Final identity lock including the separate Engine endpoint, restricted denial, cold-start and recovery against the intended exact bytes. Final changes require affected checks again. |
...
| T1–T3 temporal history | [T1 draft](../workspace/OBSERVATION-HISTORY-CONTRACT.md) defines observation ordering, current authorization, explicit retention, deletion, replay and migration requirements. No durable observation store or accepted T2 matrix exists yet. | Define and implement observation records, authorization, retention/deletion/replay/migration and disabled-by-default storage; prove known-at retrieval from retained records rather than file timestamps. |
