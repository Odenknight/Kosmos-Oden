...
| --- | --- | --- |
| R0 / R3 exact runtime and Hermes | Installed drawer package is `db48c12`; newer source and Engine pin are not installed. Earlier Hermes receipts cover earlier bytes. | Freeze and install the final package; identify client/SDK; execute every required discovery, search, allowed UID read, restricted denial, traversal, cold/warm and recovery case against those bytes. |
...
| B1 PR backlog | The September 12 disposition preserves inspected heads and records PR55/23 closure without claiming PR55 merged. | Refresh the specified PR heads/states and account for all unique behavior in 27/28/38/41 before final disposition. |
...
| W1–W4 workspace | Search, preview, origin records, structured evidence declarations, effective header, source details, lineage, map, drawers, inspector tabs and readable selection return are implemented with browser coverage. | Unresolved-reference treatment, stable UID selection and complete accepted fixture matrix; native view/edit/return/layout/performance qualification. Evidence targets remain unverified and unresolved in the inspector. |
...
| G7 / release | Candidate commits and individual receipts are published. | Exact final artifact, migration/rollback rehearsal, operational ownership, all required gates, final debug sweep and reviewed main merge. |
