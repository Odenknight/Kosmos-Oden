...
| G5 / G6 | Engine `5f96a71` includes authenticated service queries, authorized source-manifest reconciliation, revision invalidation and a persistent loopback query host. An isolated synthetic deployment passed five live HTTP queries with expected facts/citations, unauthorized/wrong-binding denial, unchanged graph counts and restart with the same published generation. Returned evidence matched the original Engine source manifest. Kosmos remains pinned to `7f28b2a`; its candidate UI/client work is not installed. | Connect the deployed reader through the Engine host adapter and actual native client; qualify complete live source/policy/configuration invalidation, resolved citations, governed UI, outage/native fallback and final artifacts. Synthetic deployment and component success do not establish production vault retrieval. |
