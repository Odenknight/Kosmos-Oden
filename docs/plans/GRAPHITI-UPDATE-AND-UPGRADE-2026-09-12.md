...
| Source-byte evidence | Engine helper merged; opt-in Kosmos capture implemented in this candidate | Full UTF-8 revision match, SHA-256, bounded page reads and policy checks; see source-evidence report |
...
| Agent display identity | Implemented and direct native MCP verified | Explicit HTTP header / stdio variable; JEFFREY's actual Hermes configuration and native acceptance remain open |
| G2 compatibility/contracts | Partial | Complete broker contract and model artifact digests still required; current receipt schemas are narrower |
