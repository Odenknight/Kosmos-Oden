...
Carl seq 10 corrects seq 9: the modern cacheable-result set is `tools/list`, `prompts/list`, `resources/list`, `resources/read`, and `resources/templates/list`. The baseline instead includes `server/discover` and omits `resources/read`. The modern revision removes `ping`. Test normative membership independently of the production set; do not implement otherwise unsupported resource methods merely to exercise this contract. Verify exact schema behavior before claiming every extra property is prohibited. [Official revision changelog](https://modelcontextprotocol.io/specification/2026-07-28/changelog).

The narrower normative lookup has been delivered. The broader independent conformance matrix remains outstanding. Preserve the established validation-first behavior: missing header `-32020`, missing body metadata `-32602`, unsupported revision `-32022`, and valid-modern `initialize` as unsupported method `-32601`. Replace tests that use `ping` merely as a metadata carrier with an appropriate implemented method, preserving their original assertion. The unused local `-32021` constant is a cleanup decision, not proof that the specification lacks that code.
