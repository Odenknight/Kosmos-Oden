# The Evolution of GKX Note Formatting — and How the Safeguards Held
...
`src/core/gkx-migration.ts`, the plugin-side apply path in
`src/plugin/gkx-migration.ts`, or the test suite that exercises both.
...
- **Every write is previewed first, as a hash-bound plan.** Scanning a vault
  never rewrites a note. It produces an `GkxMigrationPlan` — a plan hash, a
  ...
  Applying a plan re-verifies it (`verifyGkxMigrationPlan`) and refuses to
  proceed if the plan or the in-memory note contents changed since the
  preview (`src/plugin/gkx-migration.ts`, `applyGkxMigrationPlan`).
- **Byte-exact backups precede every touched file.** Before any note is
  modified, `applyGkxMigrationPlan` reads the current file with
  `readBinary` and writes an identical copy to
  `.gkx/backup/<run-id>/<path>.bak` with `writeBinary`. If a backup for that
...
- **Body bytes are never touched — only frontmatter.** Every proposal
  function (`proposedGkx`, `proposedNativeGkx23`, `proposedEditableGkx22`)
...
- **Plans and results are persisted for audit.** Every run writes
  `.gkx/migrations/<run-id>/plan.json` (content-free — note bodies are
  stripped before persisting) and `.gkx/migrations/<run-id>/result.json`
...
with a converter that wrote the full nested GKX 2.3 structure directly into
...
keep editing, which is exactly the property GKX 2.2 and 2.2-shaped 2.3 exist
to protect.
...
The audit path (`auditOne` in `src/core/gkx-migration.ts`) checks, for every
note declaring `gkx_version: "2.3"`, whether its frontmatter matches
...
path; the code comment on `proposedEditableGkx22` states this directly: the
...
For every matched note, `proposedEditableGkx22` rebuilds a flat, Properties-
editable GKX 2.2 frontmatter block: scalar fields for identity, type,
...
same `applyGkxMigrationPlan` that every migration and conversion mode uses.
Plans and results for this run are persisted under
`.gkx/migrations/<run-id>` alongside byte-exact backups under
`.gkx/backup/<run-id>`, exactly like any other run.
...
boilerplate removed (`test/gkx-migration.test.mjs`).
...
- **No writer in this engine emits nested governance blocks into a note
  ...
  2.3," which calls `proposedNativeGkx23` — a function that, like
  `proposedEditableGkx22`, only ever emits scalars and flat quoted-wikilink
  lists. `gkx_version` changes to `"2.3"` and a handful of new flat scalars
...
- **The flat Agent-Ready 2.3 profile keeps the same grammar as 2.2.** Per
  `docs/GKX-22-VS-23-POSITIONING.md`, the visible delta between a 2.2 note
...
- **The 2.3 governance model itself did not go away — it moved.** Origin
  ...
  doesn't state explicitly, with `.gkx/` sidecars as the next design cycle's
  home for values that need to persist outside the note (see
  `docs/GKX-MIGRATION.md` and the beta.13 design documents referenced
...
- **Round-tripping is enforced, not just claimed.** Notes converted during
  ...
  tested property: `test/gkx-migration.test.mjs` asserts
...
- **Byte-exact, per-file backups under `.gkx/backup/<run-id>`** meant every
...
machine-generated state. GKX 2.2 and flat Agent-Ready 2.3 are two names for
