...
- **Safer local server.** The (optional, off-by-default) Agent API now rejects requests whose `Host`/`Origin` don't point at your own machine (DNS-rebinding protection) and compares tokens in constant time. **NOTE** LAN mode uses unencrypted HTTP. Use only on a trusted network, **or** place it behind TLS, a VPN, SSH forwarding, or a mutually authenticated reverse proxy for better security.
