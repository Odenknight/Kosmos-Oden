...
test("reconciliation refuses corrupt persisted state and never clears forcing conditions", () => {
  const empty = clearReconciliationIntent(createReconciliationIntent("manual"));
  const base = { expected: snapshot(), current: snapshot(), intent: empty,
    dependencySetComplete: true, journalValid: true, checkpointValid: true, unresolvedRecovery: false };
  for (const override of [{ journalValid: "true" }, { checkpointValid: 1 }, { unresolvedRecovery: undefined }]) {
    assert.equal(evaluateReconciliation({ ...base, ...override }).classification, "block");
  }
  assert.equal(evaluateReconciliation({ ...base, dependencySetComplete: false }).classification, "full");
  assert.equal(evaluateReconciliation({ ...base, intent: { ...empty, fullRequired: true } }).classification, "full");
  assert.notEqual(evaluateReconciliation({ ...base, intent: { ...empty, affectedScopes: ["Maps"] } }).classification, "safe");
  for (const intent of [
    { ...empty, revision: Number.MAX_SAFE_INTEGER },
    { ...empty, affectedScopes: ["../outside"] },
    { ...empty, affectedScopes: ["C:/outside"] },
    { ...empty, reasons: ["unknown"] },
    { ...empty, fullRequired: "false" },
  ]) {
    assert.throws(() => mergeReconciliationIntent(intent, "manual"));
    assert.throws(() => clearReconciliationIntent(intent));
  }
