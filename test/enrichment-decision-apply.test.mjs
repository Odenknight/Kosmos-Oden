...
const obsidianStub = {
  ...
  setup(builder) {
    // Semantic apply tests inject a no-replace memory store. The real desktop
    // filesystem boundary has its own exclusive-publication tests.
    builder.onResolve({ filter: /gkx-proposal-storage$/ }, () => ({ path: "storage", namespace: "storage-stub" }));
    builder.onLoad({ filter: /.*/, namespace: "storage-stub" }, () => ({ loader: "js", contents: "export const proposalStorageAdapter = app => ({ ...app.vault.adapter, mkdir: path => app.vault.createFolder(path) });" }));
