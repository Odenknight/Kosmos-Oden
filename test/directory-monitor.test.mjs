...
for (const outcome of ["resolve", "reject"]) for (const lifecycle of ["stop", "restart", "pause-resume"]) {
  test(`${lifecycle} during a pending directory scan suppresses stale ${outcome} callbacks`, async () => {
    ...
    try {
      ...
      if (lifecycle === "pause-resume") { monitor.pause(); monitor.resume(); }
      else { monitor.stop(); if (lifecycle === "restart") monitor.start(); }
