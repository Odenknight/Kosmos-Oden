...
export type NativeSemanticProfile = Pick<Parameters<typeof prepareNativeSemanticClient>[0], "endpoint" | "token" | "authority" | "publication"> & {vaultIdentity:string};
...
export function readNativeSemanticProfile(value: unknown): NativeSemanticProfile | null {
  try {
    ...
    const keys = ["endpoint", "token", "authority", "publication", "vaultIdentity"];
    ...
    if (!["http:","https:"].includes(endpoint.protocol) || endpoint.username || endpoint.password || endpoint.hash || endpoint.search ||
        ...
        typeof profile.vaultIdentity !== "string" || !profile.vaultIdentity || profile.vaultIdentity.length > 4096) return null;
    return profile;
  } catch { return null; }
}
...
export async function prepareNativeSemanticClient(options: {
  ...
  try {
    const {api, endpoint, token, current: hostCurrent, fetcher, transportBudget} = options;
