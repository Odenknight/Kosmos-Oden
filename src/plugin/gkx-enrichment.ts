...
import { proposalStorageAdapter } from "./gkx-proposal-storage";
...
async function saveProposalQuarantine(app: App, records: GkxEnrichmentRecord[]): Promise<{ created: number; unchanged: number; path: string }> {
  ...
  const result = await persistImmutableProposals(proposalStorageAdapter(app), proposals);
...
class GkxEnrichmentPreviewModal extends Modal {
  ...
  private async buildApplyPlan(): Promise<void> {
    ...
    await saveProposalQuarantine(this.app, this.reviewRecords);
  ...
  private acceptSelected(): void {
    const candidates = this.visibleProposals().filter((item) => this.selected.has(item.proposalId));
    const acceptedIds = new Set(candidates.map(item => item.proposalId));
    new ConfidenceBatchPreviewModal(this.app, candidates, "accept", () => {
      for (const record of this.reviewRecords) record.suggestions.forEach((_suggestion, index) => { const proposal = this.proposal(record, index); if (proposal && acceptedIds.has(proposal.proposalId)) (this.controls.get(this.key(record, index))!).decision = "accepted"; });
  ...
  onOpen(): void {
    ...
    new Setting(contentEl)
      ...
      .addButton((button) => button.setButtonText("Save immutable proposals").onClick(async () => {
        try { const saved = await saveProposalQuarantine(this.app, this.result.records); new Notice(`Kosmos-Oden: ${saved.created} proposals saved to ${saved.path}; ${saved.unchanged} already existed unchanged. No note or decision was changed.`, 10000); }
        catch (error: any) { new Notice(`Proposal save stopped: ${String(error?.message || error)}`, 12000); }
      }))
      .addButton((button) => {
        this.planButton = button.buttonEl;
        button.setButtonText("Save proposals and preview changes").setWarning().onClick(async () => {
