...
window.addEventListener("message", (ev: MessageEvent) => {
  ...
  try {
    ...
    if (raw.protocol === "vault-kosmos") {
      ...
      else if (msg.type === "agent-traversal") app.notifyAgentTraversal((msg.payload as any).paths, (msg.payload as any).tool, (msg.payload as any).agent, false, (msg.payload as any).agent_id);
    ...
    else if (raw.type === "kosmos:agent" && Array.isArray(raw.paths)) app.notifyAgentTraversal(raw.paths, raw.tool || "", raw.agent, false, raw.agent_id);
