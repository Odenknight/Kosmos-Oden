...
type AgentSession = {
  name: string;
  /** Opaque renderer identity, minted independently of the MCP transport session. */
  visualId: string;
  at: number;
...
export class KosmosAgentServer {
  ...
  private registerSession(name: string, protocolVersion: string): string {
    ...
    // Never reuse or derive renderer identity from Mcp-Session-Id. The two
    // independently minted values have separate trust and exposure domains.
    const visualId = `mcp-agent:${makeToken().slice(0, 22)}`;
  ...
  async mcpDispatch(
    ...
    try {
      ...
      if (method === "tools/call") {
        ...
        try {
          const result = await this.callTool(params.name, args, ctx?.agent, ctx?.session?.visualId);
