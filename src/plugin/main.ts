...
class KosmosView extends ItemView {
  ...
  agentTraversal(paths: string[], tool: string, agent?: string, agentId?: string): void {
    if (this.ready) this.post(wrap("agent-traversal", { paths, tool, agent, agent_id: agentId }));
  }
...
export default class VaultKosmosPlugin extends Plugin {
  ...
  async onload(): Promise<void> {
    ...
    this.provider = new VaultDataProvider(this.app, this.agentSettings);
