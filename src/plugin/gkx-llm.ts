...
import { isGkxLoopbackHost, isGkxPrivateLanIpLiteral } from "gkos-engine";
import type { AgentSettings } from "./agent-server";

export function validatedGkxLlmEndpoint(provider: AgentSettings["gkxEnrichmentProvider"], raw: string): string {
  ...
  if (provider === "local" && !isGkxLoopbackHost(url.hostname)) throw new Error("On-device model endpoints must use localhost, 127.0.0.0/8, or ::1. Choose LAN LLM for a private-network model.");
  if (provider === "lan" && !isGkxPrivateLanIpLiteral(url.hostname)) throw new Error("LAN model endpoints must use a literal RFC1918, IPv4 link-local, IPv6 ULA, or IPv6 link-local address. DNS hostnames, public IPs, bind-all, and loopback addresses are rejected.");
...
export function validateGkxLlmConfiguration(settings: AgentSettings): string {
  if (!settings.gkxEnrichmentModel.trim()) throw new Error("A model name is required.");
  const endpoint = validatedGkxLlmEndpoint(settings.gkxEnrichmentProvider, settings.gkxEnrichmentEndpoint);
  const envName = settings.gkxEnrichmentApiKeyEnv.trim();
  const key = envName ? String((globalThis as any).process?.env?.[envName] || "") : "";
  if (settings.gkxEnrichmentProvider === "cloud" && !key) throw new Error(`Cloud API key environment variable ${envName || "<unset>"} is unavailable to Obsidian.`);
  return endpoint;
}
...
export async function requestGkxLlmJson(
  settings: AgentSettings,
  system: string,
  payload: unknown,
  maxTokens = 1200,
): Promise<unknown> {
  const endpoint = validateGkxLlmConfiguration(settings);
  const envName = settings.gkxEnrichmentApiKeyEnv.trim();
  const key = envName ? String((globalThis as any).process?.env?.[envName] || "") : "";
  ...
  const timeout = new Promise<never>((_, reject) => { timer = setTimeout(() => reject(new Error("Model request timed out; no retry was attempted.")), settings.gkxEnrichmentTimeoutMs); });
