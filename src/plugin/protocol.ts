...
export interface AgentTraversalPayload {
  ...
  agent?: string;
  /** Opaque visual identity minted by the server. It is deliberately distinct
   *  from transport/session credentials and conveys no authority. */
  agent_id?: string;
}
...
export function validateHostMessage(data: unknown): ValidationResult<HostToRenderer> {
  ...
  if (m.type === "agent-traversal") {
    ...
    if (p.agent_id != null && !isStr(p.agent_id)) return { ok: false, reason: "agent-traversal payload.agent_id must be a string" };
    return { ok: true, message: m as any };
  }
