...
mod tests {
    ...
    #[test]
    fn test_variant_str_iter_security_backport_mixed_direction() {
        let v = Variant::array_from_iter::<String>([
            "first".to_string().to_variant(),
            "second".to_string().to_variant(),
            "third".to_string().to_variant(),
            "fourth".to_string().to_variant(),
        ]);

        let mut iter = v.array_iter_str().unwrap();
        assert_eq!(iter.next(), Some("first"));
        assert_eq!(iter.next_back(), Some("fourth"));
